...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        first = scheme_eval(first, env) if isinstance(first, Pair) else env.lookup(first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        val = scheme_eval(expressions.rest.first, env) if isinstance(expressions.rest.first, Pair) else expressions.rest.first
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            while clause.rest is not nil:
                clause = clause.rest
                ret = scheme_eval(clause.first, env)
            return ret
            # END PROBLEM 13
        expressions = expressions.rest
    return None
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names, values = Pair(name, names), Pair(scheme_eval(expr, env), values)
