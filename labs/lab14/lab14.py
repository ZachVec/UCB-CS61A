...
def num_splits(s, d):
    ...
    def wrapped(s, n):
        ...
        first, rest = s[0], s[1:]
        # add first element to first list or second list
        return wrapped(rest, n + first) + wrapped(rest, n - first)
    return wrapped(s, 0) // 2
...
def reverse(lst):
    ...
    """
    """ Solution 1
    helper = lambda l, r: l if r is Link.empty else helper(Link(r.first, l), r.rest)
...
def foldr(link, fn, z):
    """ Right fold
    >>> lst = Link(3, Link(2, Link(1)))
    >>> foldr(lst, sub, 0) # (3 - (2 - (1 - 0))), also sub(3, sub(2, sub(1, 0)))
    2
    >>> foldr(lst, add, 0) # (3 + (2 + (1 + 0)))
    6
    >>> foldr(lst, mul, 1) # (3 * (2 * (1 * 0)))
    6
    """
    if link is Link.empty:
        return z
