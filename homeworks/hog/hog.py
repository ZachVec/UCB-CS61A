...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    outcome, k, no_one = 0, 0, True
    while k < num_rolls:
        temp = dice()
        outcome, k = outcome + temp, k + 1
        if temp == 1:
            no_one = False
    return outcome if no_one else 1
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        return GCD(opponent_score, player_score) >= 10
    # END PROBLEM 4a
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    roll_num, temp_max_scoring_num_rolls, max_average = 1, 0, 0.0
    while roll_num < 11:
        average = make_averaged(roll_dice, trials_count)(roll_num, dice)
        if max_average < average:
            temp_max_scoring_num_rolls = roll_num
            max_average = average
        roll_num = roll_num + 1
    return temp_max_scoring_num_rolls
