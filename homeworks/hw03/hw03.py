...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_arm, left_m = left(m), end(left(m))
        right_arm, right_m = right(m), end(right(m))
        return length(left_arm) * total_weight(left_m)== length(right_arm) * total_weight(right_m) and balanced(left_m) and balanced(right_m)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) > 0 or upper_bound(y) < 0, 'divisor interval must not contain 0'
...
def quadratic(x, a, b, c):
    ...
    """
    extreme_point = - b / (2 * a)
    print("DEBUG:", extreme_point, '[', lower_bound(x), ',', upper_bound(x), ']')
    fn = lambda t: a * t * t + b * t + c
    left, right = fn(lower_bound(x)), fn(upper_bound(x))
    if lower_bound(x) < extreme_point < upper_bound(x):
        return interval(fn(extreme_point), max(left, right)) if a > 0 else interval(min(left, right), fn(extreme_point))
    else:
